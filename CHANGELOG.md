...
- **README repositioned around the measured value.** The core (and the focus) is the Cyrillic doc cache + English `CLAUDE.md` — with the honest economics (translation is an investment per doc version, serving is the return; frequently-edited docs break even, code-heavy doc-light workflows gain ~nothing) and `claude-translate report` promoted so users pull their own numbers. Display translation, the headless `agent` wrapper, MCP tools and audits moved under "Extras (opt-in, off by default, experimental)", with an explicit note that all of them are toggled in the user's local `~/.claude/translate-proxy/config.yaml`.
